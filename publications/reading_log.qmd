...
# NHS-R Seminar: "An Improved I Chart" by Jacob Anhoj

13/08/2025

SPC: statistical process control

Control chart: measuring things over time

Common cause variation: normal noise - nothing to worry about

Special cause variation: non-random variation - something to worry about

The control chart doesn’t tell you why something happened but it can flag what you should investigate.

But where do you put the thresholds? How do you know when something is outside of normal limits?

I-prime (I') chart: a chart with thresholds that adapt to the size of the data, rather than remaining static over time.

Example: median glycated haemoglobin across Denmark seemed to spike in April 2020 when an I chart is plotted. But the spike is actually because there was less data that month: the country was in a Covid lockdown and fewer patients visited clinics. When you use an I-prime chart it shows that the variation is within the expected range.

# Daniel Wieand

We have lots of data.
Projects lose momentum because the tools (Excel) require you to start from scratch every time you want to redo something.

revealjs for making slides

# [Vignette: Programming with dplyr](https://dplyr.tidyverse.org/articles/programming.html)
...
# ["A personal history of the tidyverse"](https://hadley.github.io/25-tidyverse-history/)

"Unlike our universe, the tidyverse didn’t start with a big bang, but rather emerged through a gradual accumulation of packages that eventually coalesced into something greater than the sum of its parts."

"The hardest part of collaborative data analysis is not finding the correct statistical model, but getting the data into a form that you can actually work with."

"During my time at Rice I had very little success with grants. The statistics section at the National Science Foundation (NSF) had a hard time understanding why my work was important,"

"The design of dplyr was centred around verbs like filter(), mutate(), and summarise(), with names that evoked their desired purpose. Each verb did one thing well, and was designed to compose with other verbs in order to solve complex problems."

"Overall, I’m still very happy with the name, but there’s one feature that I don’t like: it implies that everything outside the tidyverse is the messyverse."

"As an interesting historical anecdote, there would have been no need for ggplot2 had I discovered the pipe earlier, as ggplot was based on function composition. I correctly identified that function composition was an unappealing user interface, but if I had discovered the pipe at that time, ggplot could have used it, rather than needing to switch to + in ggplot2."

# Hadley Wickham - Claude Code for R [Youtube](https://www.youtube.com/watch?v=dOEUdhHHfJY)

He uses Claude Code for developing dbplyr and testthat.

"Claude Code is as important as git for high-quality software engineering."

"Claude Code is often better than me for writing functions."

He thinks Claude has the lead on other AI coding agents.

"Software engineering has changed irrevocably ... There is no going back."

"The good news is that this has not taken any of the joy away from programming."

It's no longer him directly writing the code - he is having ideas and then getting Claude Code to write them into R.

People are amassing crowds of agents to do the work for them.

Having high test coverage is now even more important.

We want to focus on increasing quality, not quantity.

Important point: "It's still your code. It's your responsibility."

Claude Code is a "robotic amanuensis"

The AI agent can get you where you want to go, but it can't pick the destination.

"There are a lot of big feelings around this."

"The bad old days" before RMarkdown and Quarto involved lots of copying and pasting.

Copying and pasting is always a bad idea.

"Claude Code isn't brilliant, but it is dogged" - it will grind away until it finds the solution.

He is steering the LLM - giving it hints, and disagreeing with it sometimes.

"Software engineering has changed irrevocably - regardless of how you feel about that, you have to figure out what to do."

"The relative value of writing code has decreased, whilst the relative value of reading code has increased."

You can trick yourself into thinking that you are getting more done with an LLM, when actually it is slowing you down.

# Davis Vaughan: [Existential crisis! Is my degree worthless?](https://www.youtube.com/watch?v=5kTxZMSB9oo)

You need to use git with AI agents - you need to know what the AI changed, and the AI needs to know what you changed.

Git allows you to quickly "undo" any stuff that the AI does.

There are 2 ways to use AI for coding: juniors ask, seniors direct.

AI works best in areas that you already understand deeply (i.e. where you are a senior) - you need foundational knowledge and to be engaged with the process (instead of telling the AI to "do it for me")

Analogy of Mickey Mouse in the sorcerer's apprentice.

Claude Code is now the industry standard, since December 2025.

He was really spooked by Claude Code.

The ability to say "no" to an AI is a very important skill.

# Hadley Wickham: ["I wrote this talk with an LLM"](https://www.youtube.com/watch?v=rFE86rbBPw8)

"I feel my identity as a software developer is under threat"

Jagged edge - sudden change from being good to being bad

LLMs don't admit doubt

LLMs can't count the number of ns in words, or calculate big numbers

LLM auto-coompletes have not taken off.

Agentic coding: the LLM is an independent agent.

LLMs are good for translations - like translating words into regular expressions.

There is an issue with stochasticity - sometimes the LLMs do better and sometimes they do worse.
