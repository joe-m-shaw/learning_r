...
GitOps is devops with Git.

"If you work in research, but cannot push the data to GitHub, you could always work on the code and the infrastructure using synthetic data for instance."

# 16: Conclusion to part two

Docker images all run in Linux (Ubuntu) environments - but Linux can also be updated.

"Is using Docker truly the best we can do? Is the pipeline truly reproducible? Well, strictly speaking, not quite. Indeed, the base operating system inside Docker also gets updated. So if you build an image based on Ubuntu 22.04 today, and then again in 6 months, the operating system is not the same anymore, because the software it ships got updated. So even if the R package library and R version remain fixed, the operating system does not."

You can fix the Ubuntu version used in Docker by specifying a particular Docker digest.
