...
**Boolean algebra:** a form of algebra for expressing logical relationships. It was invented by George Boole, a nineteenth century teacher who never went to university. In the 1930s, Claude Shannon applied Boolean algebra to computing.
...
**IDLE:** Integrated Development and Learning Environment (named after Eric Idle).
...
**Java:** an object-oriented, general purpose programming language developed in the mid-1990s. The icon is a cup of coffee, because the name derives from Java coffee (i.e., coffee from the Indonesian country, Java).
...
**Parse:** parse can be used in two main contexts. The first is to break down something into it’s constituent parts, as in “parsing a text string”. The second is the process of a computer reading source code before it is compiled.
...
**Dependency (recursive)**: a recursive dependency is a dependency on a package which depends on other packages.
...
**Package (meta):** a meta-package is a package composed of several other packages which can be installed as one for conveniences. Examples of meta-packages are tidyverse (which contains several packages including dplyr, ggplot2 and readr) and devtools (which contains several packages including roxygen2 and testthat).
...
**Tidy evaluation:** you can refer to the names of values within dataframes without special treatment. This is a thing in the tidyverse.
...
"Data Organization in Spreadsheets" Broman and Woo
